...
public class BlogApiControllerTest {
	...
	// 테스트 관련된 것은 무조건 다 public void다.
	@BeforeEach // 모든 테스트 메서드 시작 전에
	...
	// 테스트를 수행할 메서드 작성
	@DisplayName("블로그에 글이 저장되어 있는지 테스트")
	@Test // 이 애노테이션이 붙은 메서드들은 모두 테스트한다.
	public void addArticle() throws Exception {
		...
		System.out.println("만들어진 JSON 데이터: " + requestBody);
		...
		// 첫번째 데이터의 title값이 "title"인지 테스트
		assertThat(articles.get(0).getTitle()).isEqualTo(title);
		// 첫번째 데이터의 content값이 "content"인지 테스트
...
// 테스트를 수행할 메서드 작성
	@DisplayName("블로그에 모든 글을 읽어오는지 테스트한다.")
	@Test // 이 애노테이션이 붙은 메서드들은 모두 테스트한다.
	public void findArticle() throws Exception {
		...
		ResultActions result = mockMvc.perform(get(url) // GET 전송
				.accept(MediaType.APPLICATION_JSON_VALUE)); // 데이터를 JSON형식으로 읽겠다.

		// 3. 실행된 결과를 가지고 결과가 올바른지 검증해본다.
		// then : 결과 확인
		result.andExpect(status().isOk()) // 성공했니?
				// 첫번째 데이터의 title이 "title"이니?
				.andExpect(jsonPath("$[0].title").value(title))
				// 첫번째 데이터의 content가 "content"이니?
				.andExpect(jsonPath("$[0].content").value(content));
		System.out.println("읽어온 값 : " + result);
		System.out.println("읽어온 값 : " + jsonPath("$[0]"));
	}
