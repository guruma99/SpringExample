...
public class HomeController {
	...
	public String getCookie(HttpServletRequest request, Model model) throws UnsupportedEncodingException {
		// 요청 받을때:HttpServletRequest
		// HttpServletRequest는 클라이언트의 요청 정보를 해석할 때 사용하는 객체이다.
		// 저장된 모든 쿠키를 읽는다.
	...
	public String addCookie(HttpServletResponse response) throws UnsupportedEncodingException {
		// 응답 받을때:HttpServletRequest
		// HttpServletRequest는 객체는 응답을 책임지는 객체.
		// 쿠키를 만든다. (만약에 서버가 한글 쿠키를 지원하지 않는다면 인코딩해서 저장한다.)
		Cookie cookie1 = new Cookie("name1", URLEncoder.encode("최고관리자", "UTF-8"));
		Cookie cookie2 = new Cookie("name2", "지나는이");
		//setMaxAge : 유효기간을 설정하는 것
		//60 : 초단위
