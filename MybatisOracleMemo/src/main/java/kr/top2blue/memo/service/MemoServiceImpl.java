...
@Service("/memoService")
@Slf4j // 로그를 출력하기 위하여 사용. 자동의로 log변수가 생성됩니다.
public class MemoServiceImpl implements MemoService {
	...
	public void update(MemoVO memoVO) {
		...
		try {
			if (memoVO != null) {
				//입력한 비밀번호와 저장된 비밀번호가 같은 경우만 수정을 한다.
				//1. 해당 글번호의 글을 디비에서 가져온다.
				MemoVO dbVO = memoDAO.selectById(memoVO.getId());
				//2. 가져온 글의 비번과 넘어온 비번이 일치하면 그 때 수정을 한다.
	...
	public void delete(MemoVO memoVO) {
		...
		try {
			if (memoVO != null) {
				//입력한 비밀번호와 저장된 비밀번호가 같은 경우만 삭제를 한다.
				//1. 해당 글번호의 글을 디비에서 가져온다.
				MemoVO dbVO = memoDAO.selectById(memoVO.getId());
				//2. 가져온 글의 비번과 넘어온 비번이 일치하면 그 때 삭제를 한다.
				if(dbVO!=null && dbVO.getPassword().equals(memoVO.getPassword())) {
